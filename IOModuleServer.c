...
void file_remove(char* FileName1, char* FileName2, char* FileName3, char* FileName4) {
	if (FileName1 != null)
		unlink(FileName1);
	if (FileName2 != null)
		unlink(FileName2);
...
void signalhandler(int sig) {
	file_remove("./managefifo", null, null, null);
	exit(0);
}
...
int main()
{
	...
	while (1) {
		...
		for (int i = 0; i < THREADPERWORK; i++) {
			...
			snprintf(argument->fifoFileName,FILENAMESIZE-1, "./channel/%sFIFO%d", request[1], i + 1);
			argument->number = i + 1;
...
void* filerecv(void* arg) {
	...
	if ((tempfd = open(tempFileName, O_RDWR | O_CREAT,0666)) < 0) {
		printf("fail to call open manage()\n");
		exit(0);
	}
